#!/usr/bin/env python3
# 发布脚本：将 pages 目录部署到 gh-pages 分支
# 功能：关闭 Node 进程，检查并提交 Git 更改，将 pages 发布到 gh-pages 分支
...
def publish_to_gh_pages():
    """将 pages 目录发布到 gh-pages 分支"""
    print_with_encoding("\n3. 正在将 pages 目录发布到 gh-pages 分支...")
    
    # 检查 pages 目录是否存在
    pages_path = os.path.join(os.getcwd(), 'pages')
    if not os.path.exists(pages_path):
        print_with_encoding("错误：pages 目录不存在")
        return False
    ...
    try:
        ...
        # 复制 main 分支的 pages 目录内容到 gh-pages 分支
        print_with_encoding("[进度] 复制 pages 目录内容到 gh-pages 分支...")
        main_pages_path = os.path.join(project_root, 'pages')
        print_with_encoding(f"[进度] 源 pages 目录: {main_pages_path}")
        ...
        copy_directory(main_pages_path, '.')
        ...
        print_with_encoding("成功：pages 目录已发布到 gh-pages 分支")
        return True
...
def main():
    ...
    print_with_encoding("\n=== 发布完成 ===")
    print_with_encoding("pages 目录已成功发布到 gh-pages 分支")
