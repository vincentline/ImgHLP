document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('mergeBtn').addEventListener('click', async function() {
    ...
    try {
      // 合并图片
      mergedResult = await ApiService.merge(Array.from(files));
  ...
  document.getElementById('splitBtn').addEventListener('click', async function() {
    ...
    try {
      // 切割图片
      const cutImages = await ApiService.split(mergedResult.mergedImage, mergedResult.positionData);
